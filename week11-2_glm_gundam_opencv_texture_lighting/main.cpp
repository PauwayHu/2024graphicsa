...
void myBody(){///我的身體
    glPushMatrix();
        glColor3f(1, 0, 0);///紅色的
        glutSolidCube(0.6);///myBody()///但是空心便實心Solid才能正確打光
    glPopMatrix();
 }
...
void display()
{
    ...
    glDisable(GL_TEXTURE_2D);///把貼圖功能先關掉,不然會被手的黑色貼圖汙染
    myBody();
    glEnable(GL_TEXTURE_2D);///再把貼圖功能開起來才能drawhandA()成功
    glColor3f(1,1,1);///再把紅色的改回白才能看得清楚
